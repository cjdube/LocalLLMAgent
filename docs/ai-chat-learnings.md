...
Code: `tasks/ai_chat_learnings.py` (the task), `tasks/_chat_transcripts.py`
...
```bash
.venv/bin/python -m tasks.ai_chat_learnings                    # yesterday (Claude + Gemini)
.venv/bin/python -m tasks.ai_chat_learnings --backfill 14      # each of the last 14 days, one process
.venv/bin/python -m tasks.ai_chat_learnings --date 2026-06-29  # one specific day (Claude only)
```
...
not instructions. Runs on the local model by design; `WREN_AI_CHAT_LEARNINGS_BACKEND`
