...
- `agent/session_ticket.py` — files a Claude Code session as a ClickUp Task in `designed`, with the plan attached and the first typed prompt quoted into the description. Not an entrypoint launchd runs and not a chat tool: its caller is the `clickup-ticket` skill in `~/.claude/skills/`, which shells out to this module's CLI. The logic sits here rather than in the skill so pytest covers it. Its two collaborators (`clickup.find_task_id`, `clickup.upload_attachment`) take a ClickUp id and are deliberately not chat tools; `upload_attachment` is the only call in that module that goes through neither `_get` nor `_write`, because ClickUp's attachment endpoint is multipart. [session-ticket.md](session-ticket.md)
