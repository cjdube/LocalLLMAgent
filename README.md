...
| `projects.py` | The user's local checkouts under `PROJECTS_DIR`. Deterministic, model-free scan — git freshness (remote, branch, last commit day, 30-day commit count, dirty tree) plus each project's README, CLAUDE.md and `docs/` headings, **and nothing else** (never `.env`). `list_projects` and `read_project` are the chat tools over it: both scan live and merge the nightly cached summary, so the git facts an answer quotes are true now, not up to a day stale. Being a catalogue tool, the description states outright that the list is not something the model knows — measured 0 fabricated projects over 24 live replays. See [docs/projects.md](docs/projects.md) |
