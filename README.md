...
| `chrome_history.py` | Read Chrome's local history DB for a date range (`fetch_chrome_history`). Chat gets the most-visited sites up to a size budget, with the true total and a note when the list is partial; the two daily tasks pass `max_sites=None` for the whole day |
...
| `tasks/mail_watcher.py` | Always on | Tells the phone the moment mail labelled `Wren/Watch` arrives — usually within seconds. Holds a Pub/Sub **streaming pull** open (dialled out, so **no port opens** and the tailnet-only surface is intact), then summarizes each new email in one sentence and pushes it. The model gets **no tools** on this path and never supplies the sender or subject, because an email is untrusted text. **A label covers the whole thread**, so every later reply on a labelled conversation reaches Wren with no further action — the watch itself is unfiltered and the code asks Gmail, per thread, whether the label is on it. His own replies and drafts are skipped. A quiet inbox means no pushes, which is normal. See [docs/mail-watch.md](docs/mail-watch.md). |
