...
| `docs.py` | Read-only search of Wren's *own* documentation (`search_docs` → `read_doc`) — `README.md`, `ANALYSIS.md` and the top level of `docs/`, 40 documents, so "how do you handle a scheduled task that fails" is answered from the file that decided it rather than from pretraining. Mirrors the *shape* of `wiki.py` without importing it: the two corpora share no conventions, and coupling them would let a wiki format change break this silently. **`AGENTS.md` is deliberately out** — it is imperative instructions aimed at *coding* agents, and Wren reading "run pytest before calling a change done" is an order meant for somebody else; `docs/reviews/` and `docs/handoff/` are gitignored and out too. Nine documents exceed the 12000-char budget and the README is 96KB, so a trimmed read names the sections it dropped and can be re-read one at a time via `read_doc(name, section=...)`. See [docs/self-knowledge.md](docs/self-knowledge.md) |
