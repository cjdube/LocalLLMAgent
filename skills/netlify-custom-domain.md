...
3. **Set both to "DNS only" — the grey cloud.** This is the step people miss. With
   Cloudflare's orange-cloud proxy on, Netlify can't complete the certificate
   validation challenge and never issues the Let's Encrypt certificate, so the
   site stays on a TLS error with no obvious cause.
