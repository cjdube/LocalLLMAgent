...
**Two agents live here.** Wren (`agent/`, `chat/`, `tasks/`) is interactive: she reads the record and acts on request. **Scribe** (`scribe/`) is the journaling agent: it writes the record. The seam is one sentence — **Scribe writes the record, Wren reads it** — which is why the raw-capture tools are not in Wren's registry. [docs/scribe.md](docs/scribe.md)
...
- `scribe/*.py` — Scribe's tasks (`local.scribe.*`). A **pipeline** agent: gather → one `complete_text()` call → write. No tool registry, never `advance()`. It imports only the porch listed in `scribe/__init__.py` — never `agent.toolset` or `chat.*` — and **nothing under `agent/`, `chat/` or `tasks/` may import `scribe.*`** (`evals/` excepted). The porch is what would travel with Scribe if it is ever extracted, so adding to it is a decision, not a drive-by import. Its model dial is its own: `SCRIBE_<TASK>_BACKEND` → `SCRIBE_LLM_BACKEND` → ollama, with no `WREN_*` fallback. [docs/scribe.md](docs/scribe.md)
...
- **New scheduled task**: `tasks/<name>.py` — or `scribe/<name>.py` if it is journaling, i.e. it records what was done — with `main() -> int`, `setup_logger` and `notify_failure` from `tasks/_common.py`, plus a launchd plist (`local.wren.*` or `local.scribe.*` to match). **Log `Starting <name> run` on entry and `<name> run complete` on every success path** (and `logger.error` on the failure path — `notify_failure` doesn't log): `chat/insights.py:parse_runs` builds the dashboard's run history from those lines, never from exit codes, so a task without them reads as *has not run* and one with no ending hangs as *running*. `mail_watch_renew` renewed the Gmail watch every morning and showed as never-run for weeks. Polling jobs (`StartInterval`, no `StartCalendarInterval`) count as daemons and are excluded on purpose — leave them out. `tasks/morning_brief.py` is the digest template; `scribe/daily_chrome_learnings.py` the gather→LLM→persist one.
