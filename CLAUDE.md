...
- `chat/` — Flask chat server (phone-reachable UI + API); pauses on gated writes for tap-to-confirm. **Only** the conversation engine, auth, and the static view pages live in `chat/server.py`. A new API gets a `chat/routes_*.py` blueprint, never another route in `server.py`; a new **page** is one entry in the `VIEW_PAGES` table, never a hand-written handler — nine copies of the same four lines is what made this a rule.
- `agent/loop.py` — model interface: `advance()` (tool-calling loop), `complete_text()` (one-shot, tool-free — what scheduled tasks use). Both route through the `_llm_chat` backend seam; `agent/backends/` holds the opt-in adapters, which translate to/from their provider format so callers speak one canonical shape.
- `agent/toolset.py` — the single tool registry (`TOOLS`, `DISPATCH`) and gating sets (`WRITE_TOOLS`, `CONSEQUENTIAL_TOOLS`, `UNATTENDED_EXCLUDED_TOOLS`), shared by the chat server and background runs; one module per capability in `agent/tools/*.py`.
- `agent/store.py` — locked/atomic JSON store primitives used by every store under `config/`.
- `tasks/*.py` — unattended entrypoints run by launchd, with shared helpers in `tasks/_common.py` and `tasks/_learnings_common.py`; `launchd/` holds one plist per task, logging to `logs/`.
- `tests/` — flat pytest suite, one `test_<module>.py` per source module, except where a module sits behind a seam and is tested through it. Plus flat jest/jsdom suites (`npm test`), one `tests/<script>.test.js` per standalone browser script in `chat/static/`.
...
- **A source's timestamps are UTC until proven otherwise; our day windows are local.** Convert before comparing or truncating — never slice an ISO stamp (`published_at[:10]`) and match it against a local calendar day. Use `local_timezone()` from `agent/dates.py`; `_liked_local_date` in `agent/tools/youtube.py` is the pattern. Bitten three times: the two dates agree until after 8pm local, so the bug passes review and a midday spot-check, then silently misfiles evening data instead of raising. Any test covering this pins `TIMEZONE` (`monkeypatch.setenv`) and uses an evening timestamp. [docs/timezones.md](docs/timezones.md)
...
- **Deterministic Python owns structure.** Date/duration math, timestamps, URL handling, and HTML assembly happen in Python — never ask the model for a date or let it freehand a whole email/digest. **Weekdays are date math too**: a day-taking tool gets the user's phrase verbatim (`'next tuesday'`) and resolves it in `agent/dates.py`, and returns the day it used so the reply quotes the tool. Asked on a Friday for "next Tuesday" the model looked up the Wednesday, found nothing, and reported the wrong date as fact — nothing failed, so nothing logged. [docs/model-constraints.md](docs/model-constraints.md)
...
- **Pass `think=False` for any call that fills in a template** — a classification, a score, a fixed output format — and pass `logger=` with it. Thinking tokens share the `num_predict` budget with the answer, so a call that reasons too long returns *empty content*, not a truncated answer. Measured on 40-lead scoring: thinking on, 0 of 3 runs produced output; off, 3 of 3 and 5x faster. Leave it on only where the model must reason past what the prompt already contains (`evaluate_app`, `daily_synthesis`) — "is this analysis?" is *not* the test, and measure even then. On Gemini the knob is `WREN_GEMINI_THINKING_BUDGET`. [docs/model-constraints.md](docs/model-constraints.md)
- **Degrading on bad model output is only safe if it's logged.** "Degrade, don't crash" suits a dead feed, whose absence is obvious in the digest; applied to model output it hides the bug. If a parse yields *fewer* results than inputs — not just zero — log WARNING with the counts and the raw length. A scheduled task that silently produces *less* is worse than one that fails, because only the failure pushes an alert. Three instances, each silent for weeks: [docs/model-constraints.md](docs/model-constraints.md)
- **Never make the model copy an opaque identifier.** Number the items (`{"n": 1, ...}`) and map back to ids in Python. Google event ids and `lever:<slug>:<uuid>` lead ids sent for round-tripping were transcribed character by character until the budget ran out — and were mis-copied even on runs that succeeded, so those items silently matched nothing. [docs/opaque-identifiers.md](docs/opaque-identifiers.md)
- **A tool that answers "what exists?" has to say the answer isn't in the model's head.** A catalogue question is the one shape where pretraining supplies a *plausible* answer, so the model skips the tool and invents one. Saying only *when* to call it isn't enough: the description must say the list is **not something you know**, that only what the tool returns exists, and what to say when it returns nothing. Took `list_games` from 2-of-12 replays fabricating games to 12 of 12 calling the tool; wording pinned by a test in `tests/test_games.py`. Applies to any registry-style tool. [docs/model-constraints.md](docs/model-constraints.md)
- **A confirmation-gated chain must be bounded in code, not by the model stopping.** A gated call returns out of `advance()`, so `MAX_TOOL_ITERATIONS` resets on every continuation: one "add a calendar event" produced four cards, and *declining* fed the next one because a decline came back as `{"error": ...}` — the shape a crashed tool returns, which a model correctly retries. A tool result must also carry `tool_name` and say what it wrote, or the model can't tell its call ran. Guards in `agent/loop.py` (`MAX_GATED_PAUSES_PER_TURN`, `_answered_gated_calls`) are the guarantee, not the prompt wording — the model-level trigger never reproduced in 16 live runs. [docs/limits.md](docs/limits.md)
- **Never tell the model to *describe* an action it should *perform*.** It describes and stops — no tool_call, so nothing is written, gated, or logged, and the reply is shaped exactly like a legitimate one. Any instruction about a confirmation-gated tool must say *call the tool in the same turn*, and that the pause is the app's job, not a reason to wait for a verbal go-ahead. Rewording `agent/wren_chat.md` that way took the replay from 2-of-3 failing to 9 of 9. Backstop: `chat/server.py:_warn_if_promised_without_acting`. [docs/model-constraints.md](docs/model-constraints.md)
...
Three real incidents: tests wrote fixture rows into production `logs/`, sent real ntfy pushes to the phone, and — worst — a daemon thread spawned by a server test outlived its test and saved fixture data over the production `config/opportunities.json`. Rules, enforced by autouse fixtures in `tests/conftest.py`:
