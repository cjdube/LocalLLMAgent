...
- **Pass `think=False` for any call that fills in a template** — a classification, a score, a fixed output format. The model's thinking tokens come out of the same `num_predict` budget as its answer, so a call that reasons too long returns *empty content*, not a truncated answer, and the caller reports it as a parse failure. This has bitten twice (`calendar_colorizer` crashed on an empty response; `opportunity_digest` silently emailed 11 unscored leads). Measured on 40-lead scoring: thinking on, 0 of 3 runs produced output; thinking off, 3 of 3 and 5x faster. Leave thinking on only where the reasoning *is* the deliverable (`evaluate_app`, `evaluate_against`, `research`, `daily_synthesis`). Pass `logger=` too — `agent/loop.py` warns when a generation hits the cap, but only if it has a logger. Only the Ollama path acts on `think`; on Gemini the equivalent knob is `WREN_GEMINI_THINKING_BUDGET` (already 0 by default, and some models reject 0 — see `agent/backends/gemini.py`).
