...
- `tests/` — flat pytest suite, one `test_<module>.py` per source module; plus the jest/jsdom suites (`npm test`) covering the standalone browser scripts, one `<script>.test.js` per script in `chat/static/`.
