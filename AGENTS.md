...
**One agent lives here.** Wren (`agent/`, `chat/`, `tasks/`) is interactive: she reads the record and acts on request. The journaling agent **ScribeJay** left for its own repo (`~/Projects/ScribeJay`) on 2026-08-30. The seam it left behind still binds changes here — **ScribeJay writes the record, Wren reads it** — which is why the raw-capture tools are not in Wren's registry. Knowledge runs one way: Wren federates ScribeJay's runs onto her dashboard; nothing here may import from that checkout, and nothing there knows Wren exists. [docs/scribejay.md](docs/scribejay.md)
...
- **`~/Projects/ScribeJay` is a sibling repo, not a subdirectory.** Nothing here may import it, shell into it, or write to its config; it is reachable only as data (vault pages, calendar events, its logs, its installed `local.scribejay.*` launchd agents, and one key of `~/.scribejay/config.json`). It surfaces on the dashboard through `WREN_EXTERNAL_TASK_ROOTS` and nothing more. **Its plists are generated into `~/Library/LaunchAgents`, not committed** — federating only `<root>/launchd` took all eight routines off the dashboard on 2026-08-30. Its `SCRIBEJAY_*` variables are inert in this repo's `.env`. [docs/scribejay.md](docs/scribejay.md)
...
- **New scheduled task**: `tasks/<name>.py` with `main() -> int`, `setup_logger` and `notify_failure` from `tasks/_common.py`, plus a `local.wren.*` launchd plist. **If it is journaling — i.e. it records what was done — it belongs in the ScribeJay repo instead, not here.** **Log `Starting <name> run` on entry and `<name> run complete` on every success path** (and `logger.error` on the failure path — `notify_failure` doesn't log): `chat/insights.py:parse_runs` builds the dashboard's run history from those lines, never from exit codes, so a task without them reads as *has not run* and one with no ending hangs as *running*. `mail_watch_renew` renewed the Gmail watch every morning and showed as never-run for weeks. **A collaborator that reports failure by *returning* `{"error": ...}` never reaches the `except`**, so check the returned dict and `return` before the completion line — `morning_brief` logged "run complete" and showed a green row on every morning its `send_email` failed. Polling jobs (`StartInterval`, no `StartCalendarInterval`) count as daemons and are excluded on purpose — leave them out. `tasks/morning_brief.py` is the digest template; `tasks/daily_synthesis.py` the gather→LLM→persist one.
...
- **Never spawn a real background thread in a test.** Any function that spawns one (e.g. `chat/routes_opportunities.py:_start_research`) gets an autouse stub in its test file. A surviving thread resolves monkeypatched paths *after* they're restored — a passing suite is timing luck.
