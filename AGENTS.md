...
- **`~/Projects/ScribeJay` is a sibling repo, not a subdirectory.** Nothing here may import it, shell into it, or write to its config; it is reachable only as data (vault pages, calendar events, its `logs/`). It surfaces on the dashboard through `WREN_EXTERNAL_TASK_ROOTS` and nothing more. Its `SCRIBEJAY_*` variables are inert in this repo's `.env`. [docs/scribejay.md](docs/scribejay.md)
